...
accept_keys = ["HealthPoints", "NormalDamage", "SpecialDamage", 
  "PhysicalArmor", "MoveSpeed", "CastSpeed", "Shield", "Dodge", "ReviveTime"]
...
class HeroStats(Stats):
  ...
  def calculateStats(self):
    ...
    self.PhysicalArmor = self.baseArmor + self.rank*self.drankArmor
  ...
  def get_clean_stats(self, transform:bool=False):
    ...
    for key, value in self.__dict__.items():
      if key not in accept_keys or (self[key] == 0 and "PhysicalArmor" != key):
        continue
